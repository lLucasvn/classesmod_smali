.class public Lcom/google/androidgamesdk/SwappyDisplayManager;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Landroid/hardware/display/DisplayManager$DisplayListener;


# instance fields
.field private final DEBUG:Z

.field private final LOG_TAG:Ljava/lang/String;

.field private final ONE_MS_IN_NS:J

.field private final ONE_S_IN_NS:J

.field private mActivity:Landroid/app/Activity;

.field private mCookie:J

.field private mCurrentMode:Landroid/view/Display$Mode;

.field private mLooper:Lcom/google/androidgamesdk/d;

.field private mWindowManager:Landroid/view/WindowManager;


# direct methods
.method public constructor <init>(JLandroid/app/Activity;)V
    .registers 7

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const-string v0, "SwappyDisplayManager"

    iput-object v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->LOG_TAG:Ljava/lang/String;

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->DEBUG:Z

    const-wide/32 v0, 0xf4240

    iput-wide v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->ONE_MS_IN_NS:J

    const-wide/32 v0, 0x3b9aca00

    iput-wide v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->ONE_S_IN_NS:J

    :try_start_14
    invoke-virtual {p3}, Landroid/content/Context;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object v0

    invoke-virtual {p3}, Landroid/app/Activity;->getIntent()Landroid/content/Intent;

    move-result-object v1

    invoke-virtual {v1}, Landroid/content/Intent;->getComponent()Landroid/content/ComponentName;

    move-result-object v1

    const/16 v2, 0x80

    invoke-virtual {v0, v1, v2}, Landroid/content/pm/PackageManager;->getActivityInfo(Landroid/content/ComponentName;I)Landroid/content/pm/ActivityInfo;

    move-result-object v0

    iget-object v0, v0, Landroid/content/pm/ActivityInfo;->metaData:Landroid/os/Bundle;

    if-eqz v0, :cond_40

    const-string v1, "android.app.lib_name"

    invoke-virtual {v0, v1}, Landroid/os/BaseBundle;->getString(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_40

    invoke-static {v0}, Ljava/lang/System;->loadLibrary(Ljava/lang/String;)V
    :try_end_35
    .catchall {:try_start_14 .. :try_end_35} :catchall_36

    goto :goto_40

    :catchall_36
    move-exception v0

    invoke-virtual {v0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object v0

    const-string v1, "SwappyDisplayManager"

    invoke-static {v1, v0}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    :cond_40
    :goto_40
    iput-wide p1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCookie:J

    iput-object p3, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mActivity:Landroid/app/Activity;

    const-class p1, Landroid/view/WindowManager;

    invoke-static {p3, p1}, LP1/h;->a(Landroid/app/Activity;Ljava/lang/Class;)Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Landroid/view/WindowManager;

    iput-object p1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mWindowManager:Landroid/view/WindowManager;

    invoke-interface {p1}, Landroid/view/WindowManager;->getDefaultDisplay()Landroid/view/Display;

    move-result-object p1

    invoke-static {p1}, LP1/d;->a(Landroid/view/Display;)Landroid/view/Display$Mode;

    move-result-object p2

    iput-object p2, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-direct {p0, p1}, Lcom/google/androidgamesdk/SwappyDisplayManager;->updateSupportedRefreshRates(Landroid/view/Display;)V

    iget-object p1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mActivity:Landroid/app/Activity;

    const-class p2, Landroid/hardware/display/DisplayManager;

    invoke-static {p1, p2}, LP1/h;->a(Landroid/app/Activity;Ljava/lang/Class;)Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Landroid/hardware/display/DisplayManager;

    monitor-enter p0

    :try_start_66
    new-instance p2, Lcom/google/androidgamesdk/d;

    const/4 p3, 0x0

    invoke-direct {p2, p3}, Lcom/google/androidgamesdk/d;-><init>(LP1/i;)V

    iput-object p2, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mLooper:Lcom/google/androidgamesdk/d;

    invoke-virtual {p2}, Lcom/google/androidgamesdk/d;->start()V

    iget-object p2, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mLooper:Lcom/google/androidgamesdk/d;

    iget-object p2, p2, Lcom/google/androidgamesdk/d;->a:Landroid/os/Handler;

    invoke-virtual {p1, p0, p2}, Landroid/hardware/display/DisplayManager;->registerDisplayListener(Landroid/hardware/display/DisplayManager$DisplayListener;Landroid/os/Handler;)V

    monitor-exit p0

    return-void

    :catchall_7a
    move-exception p1

    monitor-exit p0
    :try_end_7c
    .catchall {:try_start_66 .. :try_end_7c} :catchall_7a

    throw p1
.end method

.method static bridge synthetic a(Lcom/google/androidgamesdk/SwappyDisplayManager;)Landroid/app/Activity;
    .registers 1

    .line 1
    iget-object p0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mActivity:Landroid/app/Activity;

    return-object p0
.end method

.method private modeMatchesCurrentResolution(Landroid/view/Display$Mode;)Z
    .registers 4

    invoke-static {p1}, LP1/b;->a(Landroid/view/Display$Mode;)I

    move-result v0

    iget-object v1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-static {v1}, LP1/b;->a(Landroid/view/Display$Mode;)I

    move-result v1

    if-ne v0, v1, :cond_1a

    invoke-static {p1}, LP1/c;->a(Landroid/view/Display$Mode;)I

    move-result p1

    iget-object v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-static {v0}, LP1/c;->a(Landroid/view/Display$Mode;)I

    move-result v0

    if-ne p1, v0, :cond_1a

    const/4 p1, 0x1

    return p1

    :cond_1a
    const/4 p1, 0x0

    return p1
.end method

.method private native nOnRefreshPeriodChanged(JJJJ)V
.end method

.method private native nSetSupportedRefreshPeriods(J[J[I)V
.end method

.method private updateSupportedRefreshRates(Landroid/view/Display;)V
    .registers 8

    invoke-static {p1}, LP1/f;->a(Landroid/view/Display;)[Landroid/view/Display$Mode;

    move-result-object p1

    const/4 v0, 0x0

    const/4 v1, 0x0

    const/4 v2, 0x0

    :goto_7
    array-length v3, p1

    if-ge v1, v3, :cond_18

    aget-object v3, p1, v1

    invoke-direct {p0, v3}, Lcom/google/androidgamesdk/SwappyDisplayManager;->modeMatchesCurrentResolution(Landroid/view/Display$Mode;)Z

    move-result v3

    if-nez v3, :cond_13

    goto :goto_15

    :cond_13
    add-int/lit8 v2, v2, 0x1

    :goto_15
    add-int/lit8 v1, v1, 0x1

    goto :goto_7

    :cond_18
    new-array v1, v2, [J

    new-array v2, v2, [I

    const/4 v3, 0x0

    :goto_1d
    array-length v4, p1

    if-ge v0, v4, :cond_43

    aget-object v4, p1, v0

    invoke-direct {p0, v4}, Lcom/google/androidgamesdk/SwappyDisplayManager;->modeMatchesCurrentResolution(Landroid/view/Display$Mode;)Z

    move-result v4

    if-nez v4, :cond_29

    goto :goto_40

    :cond_29
    aget-object v4, p1, v0

    invoke-static {v4}, LP1/e;->a(Landroid/view/Display$Mode;)F

    move-result v4

    const v5, 0x4e6e6b28  # 1.0E9f

    div-float/2addr v5, v4

    float-to-long v4, v5

    aput-wide v4, v1, v3

    aget-object v4, p1, v0

    invoke-static {v4}, LP1/g;->a(Landroid/view/Display$Mode;)I

    move-result v4

    aput v4, v2, v3

    add-int/lit8 v3, v3, 0x1

    :goto_40
    add-int/lit8 v0, v0, 0x1

    goto :goto_1d

    :cond_43
    iget-wide v3, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCookie:J

    invoke-direct {p0, v3, v4, v1, v2}, Lcom/google/androidgamesdk/SwappyDisplayManager;->nSetSupportedRefreshPeriods(J[J[I)V

    return-void
.end method


# virtual methods
.method public onDisplayAdded(I)V
    .registers 2

    return-void
.end method

.method public onDisplayChanged(I)V
    .registers 12

    monitor-enter p0

    :try_start_1
    iget-object p1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mWindowManager:Landroid/view/WindowManager;

    invoke-interface {p1}, Landroid/view/WindowManager;->getDefaultDisplay()Landroid/view/Display;

    move-result-object p1

    invoke-virtual {p1}, Landroid/view/Display;->getRefreshRate()F

    move-result v0

    invoke-static {p1}, LP1/d;->a(Landroid/view/Display;)Landroid/view/Display$Mode;

    move-result-object v1

    invoke-static {v1}, LP1/c;->a(Landroid/view/Display$Mode;)I

    move-result v2

    iget-object v3, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-static {v3}, LP1/c;->a(Landroid/view/Display$Mode;)I

    move-result v3

    const/4 v4, 0x0

    const/4 v5, 0x1

    if-eq v2, v3, :cond_1f

    const/4 v2, 0x1

    goto :goto_20

    :cond_1f
    const/4 v2, 0x0

    :goto_20
    invoke-static {v1}, LP1/b;->a(Landroid/view/Display$Mode;)I

    move-result v3

    iget-object v6, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-static {v6}, LP1/b;->a(Landroid/view/Display$Mode;)I

    move-result v6

    if-eq v3, v6, :cond_2e

    const/4 v3, 0x1

    goto :goto_2f

    :cond_2e
    const/4 v3, 0x0

    :goto_2f
    or-int/2addr v2, v3

    iget-object v3, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;

    invoke-static {v3}, LP1/e;->a(Landroid/view/Display$Mode;)F

    move-result v3

    cmpl-float v3, v0, v3

    if-eqz v3, :cond_3b

    const/4 v4, 0x1

    :cond_3b
    iput-object v1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCurrentMode:Landroid/view/Display$Mode;
    :try_end_3d
    .catchall {:try_start_1 .. :try_end_3d} :catchall_6c

    if-eqz v2, :cond_47

    :try_start_3f
    invoke-direct {p0, p1}, Lcom/google/androidgamesdk/SwappyDisplayManager;->updateSupportedRefreshRates(Landroid/view/Display;)V
    :try_end_42
    .catchall {:try_start_3f .. :try_end_42} :catchall_43

    goto :goto_47

    :catchall_43
    move-exception v0

    move-object p1, v0

    move-object v1, p0

    goto :goto_72

    :cond_47
    :goto_47
    if-eqz v4, :cond_6f

    :try_start_49
    invoke-virtual {p1}, Landroid/view/Display;->getAppVsyncOffsetNanos()J

    move-result-wide v6

    iget-object p1, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mWindowManager:Landroid/view/WindowManager;

    invoke-interface {p1}, Landroid/view/WindowManager;->getDefaultDisplay()Landroid/view/Display;

    move-result-object p1

    invoke-virtual {p1}, Landroid/view/Display;->getPresentationDeadlineNanos()J

    move-result-wide v1

    const p1, 0x4e6e6b28  # 1.0E9f

    div-float/2addr p1, v0

    float-to-long v4, p1

    const-wide/32 v8, 0xf4240

    sub-long/2addr v1, v8

    sub-long v8, v4, v1

    iget-wide v2, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mCookie:J
    :try_end_64
    .catchall {:try_start_49 .. :try_end_64} :catchall_6c

    move-object v1, p0

    :try_start_65
    invoke-direct/range {v1 .. v9}, Lcom/google/androidgamesdk/SwappyDisplayManager;->nOnRefreshPeriodChanged(JJJJ)V

    goto :goto_70

    :catchall_69
    move-exception v0

    :goto_6a
    move-object p1, v0

    goto :goto_72

    :catchall_6c
    move-exception v0

    move-object v1, p0

    goto :goto_6a

    :cond_6f
    move-object v1, p0

    :goto_70
    monitor-exit p0

    return-void

    :goto_72
    monitor-exit p0
    :try_end_73
    .catchall {:try_start_65 .. :try_end_73} :catchall_69

    throw p1
.end method

.method public onDisplayRemoved(I)V
    .registers 2

    return-void
.end method

.method public setPreferredDisplayModeId(I)V
    .registers 4

    iget-object v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mActivity:Landroid/app/Activity;

    new-instance v1, Lcom/google/androidgamesdk/c;

    invoke-direct {v1, p0, p1}, Lcom/google/androidgamesdk/c;-><init>(Lcom/google/androidgamesdk/SwappyDisplayManager;I)V

    invoke-virtual {v0, v1}, Landroid/app/Activity;->runOnUiThread(Ljava/lang/Runnable;)V

    return-void
.end method

.method public terminate()V
    .registers 2

    iget-object v0, p0, Lcom/google/androidgamesdk/SwappyDisplayManager;->mLooper:Lcom/google/androidgamesdk/d;

    iget-object v0, v0, Lcom/google/androidgamesdk/d;->a:Landroid/os/Handler;

    invoke-virtual {v0}, Landroid/os/Handler;->getLooper()Landroid/os/Looper;

    move-result-object v0

    invoke-virtual {v0}, Landroid/os/Looper;->quit()V

    return-void
.end method
